...
for index = 0:M*N*O-1
    ...
    % denoise X
    X_ = X(:,maskX);
    [X(:,maskX),s2,p] = denoiseMatrix(X_);
    % It keeps giving erros, even when there are only normal values inside the mask: TODO: figure out what happens
    % try
    %     [X(:,maskX),s2,p] = denoiseMatrix(X_);
    % catch
    %     fprintf('Error: Denoising matrix failed (non-zero entries = %d)\n', nnz(maskX));
    %     p  = 1;  % FIXME: 1 is just a dummy value
    %     s2 = 1;  % FIXME: 1 is just a dummy value
    % end
