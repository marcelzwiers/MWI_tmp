function Macro_all_MZ(bids_dir, preprocessing, SepiaPrep, fittingMCR, fittingMCRGPU, writingMCR, acqname, run_label)
% Process BIDS directory for Myelin Water Imaging (compiled version)
%
% Usage:
%   Macro_all_MZ() % uses all defaults
%   Macro_all_MZ(bids_dir, preprocessing, SepiaPrep, fittingMCR, writingMCR, fittingMCRGPU, acqname, run)
%
% Inputs:
...
if nargin == 1 && ischar(bids_dir) && (strcmpi(bids_dir, '--help') || strcmpi(bids_dir, '-h'))
    fprintf('\nMacro_all_MZ - Process BIDS directory for Myelin Water Imaging\n\n');
    fprintf('Usage:\n');
    fprintf('  Macro_all_MZ() %% uses all defaults\n');
    fprintf('  Macro_all_MZ(bids_dir, preprocessing, SepiaPrep, fittingMCR, fittingMCRGPU, writingMCR, acqname, run)\n\n');
    fprintf('Inputs:\n');
...
for subjn = 1:length(subjects)
    ...
    if preprocessing
        ProcessingPipelineModular(subj_label, run_label, bids_ses_dir, derivative_FSL_dir, derivative_SEPIA_dir, derivative_MRI_SYNTHSEG_dir)
    end

    if SepiaPrep
        SEPIA_03_standard_pipeline(subj_label, run_label, bids_ses_dir, derivative_FSL_dir, derivative_SEPIA_dir, derivative_MRI_SYNTHSEG_dir)
        script_SCR
    end
    ...
    if fittingMCRGPU
        addpath(genpath(fullfile(code_dir,'gacelle'))
